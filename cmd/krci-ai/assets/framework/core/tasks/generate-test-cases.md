...
- Require ./docs/testing/README.md and ./docs/testing/features/

Prechecks (route user appropriately):
- If `./docs/testing/features/` exists and `./docs/testing/README.md` is missing → propose running `onboard-testing` instead; HALT until user confirms.
- If both `./docs/testing/README.md` and `./docs/testing/features/` are missing → propose running `setup-testing` instead; HALT until user confirms.
...
3. **Read testing README**: Review `./docs/testing/README.md` for process, directory structure, and tagging rules (single source of truth)
4. **Scan existing coverage**: Analyze `./docs/testing/features/` per README to determine Covered / Partial / Not covered
5. **Decide actions**: Use README decision matrix; request confirmation before creating/updating tests
6. **Generate Gherkin**: Create or extend `.feature` files under `./docs/testing/features/` with proper tags and structure
