...
1. Single agent installation with dependency resolution
   ...
   - Files Created/Changed: `.krci-ai/agents/developer.yaml` and dependency files
...
2. Multi-agent installation using flexible syntax (matches bundle command)
   ...
   - Non-automatable at story-level: false
   - Files Created/Changed: `.krci-ai/agents/pm.yaml`, `.krci-ai/agents/architect.yaml`, `.krci-ai/agents/developer.yaml` and dependency files
...
3. Dependency resolution validation
   - Scenario: Given agents with dependencies, when selective installation occurs, then all required dependency files are included automatically
   ...
   - Environment/Flags: None required
   - Traceability: Epic infrastructure support; BR13, BR14

4. IDE integration for selective installations
   - Scenario: Given selective agent installation with IDE integration flag, when installation completes, then IDE configuration reflects only installed agents
   - Expected Behavior: IDE integration works seamlessly with selective installations; agent configurations match installed scope
   - Verification Command: `krci-ai install --agent developer --ide cursor && test -f .cursor/rules/krci-ai/dev.mdc` (Expect: exit 0, IDE config exists)
   - Evidence: IDE configuration files created and functional agent activation
   - Non-automatable at story-level: false
   - Files Created/Changed: `.cursor/rules/krci-ai/*.mdc` (IDE configuration files)
   - Guardrails: IDE remains stable; agent functionality preserved; configuration reversible
   - Test Data/Fixtures: Supported IDE installations and agent definitions
   - Environment/Flags: Supported IDE installed (Cursor, Claude, VS Code, Windsurf)
   - Traceability: Epic AC #3; BR15

5. Installation error handling and rollback
...
- [x] **Task 1: Implement CLI interface with selective installation flags (AC: 1, 2)**
  - [x] Edit file: `cmd/krci-ai/cmd/install.go` - added `--agent` and `--agents` flags
  - [x] Integrated ParseAgentList() from bundle command for consistent parsing
  - [x] Added validation logic for agent names
...
- `cmd/krci-ai/cmd/install.go` - Added `--agent` and `--agents` flags with validation
...
- [x] **CLI Consistency**: `../dist/krci-ai install --help` shows consistent `--agent` and `--agents` flags
...
- [x] **Foundation Complete**: Core agent installation functionality fully implemented
- [x] **Agent Selection**: Single and multi-agent installation works correctly
- [x] **Validation Chain**: Core validation supports selective installation scenarios
- [x] **Command Consistency**: `--agent` parsing established and working
- [x] **Dependency Preservation**: Core dependency resolution maintains integrity
...
- [x] **Invalid Agent Names**: Proper validation for invalid agent combinations
